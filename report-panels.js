...
function eegOverview(b,a,bands){
 const peak=Math.max(1,...bands.flatMap(([k])=>[Number(b.eeg[k])||0,Number(a.eeg[k])||0]));
 return `<article class="panel eeg-summary-panel"><div class="eeg-summary-head"><div><span>频段变化概览</span><strong>前后测相对强度</strong></div><div class="eeg-summary-legend"><i></i>前测 <i></i>后测</div></div><div class="eeg-band-list">${bands.map(([k,l])=>{const before=Number(b.eeg[k])||0,after=Number(a.eeg[k])||0;return `<div class="eeg-band-row"><b>${l}</b><div class="eeg-compare-bars"><i style="width:${before/peak*100}%"></i><i style="width:${after/peak*100}%"></i></div><span>${num(before)} <em>→</em> ${num(after)}</span></div>`}).join('')}</div><small>同一设备字段的前后相对比较，仅展示本次体验变化。</small></article>`;
}
