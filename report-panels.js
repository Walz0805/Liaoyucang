...
function trend(before=[],after=[],unit='字段值'){
 ...
 return `<svg class="signal-chart" preserveAspectRatio="none" viewBox="0 0 350 140" role="img" aria-label="前后测${escape(unit)}序列比较">${[0,.5,1].map(v=>`<path d="M38 ${15+95*v}H338" stroke="#95b7d31b"/><text x="0" y="${19+95*v}" fill="#7891ac" font-size="9">${num(hi-(hi-lo)*v)}</text>`).join('')}${poly(before,'#6a83b8')}${poly(after,'#a0dfd6')}<text x="38" y="133" fill="#7891ac" font-size="9">开始</text><text x="260" y="133" fill="#7891ac" font-size="9">归一化采样进程</text></svg>`;
}
...
export function reportPanels(b,a){
 ...
 return `<div class="stai-report"><div><span>STAI-S · 状态焦虑</span><strong>${num(bs)} <i>→</i> ${num(as)}</strong></div><div class="stai-track"><i style="width:${Math.min(100,bs||0)}%"></i><i style="width:${Math.min(100,as||0)}%"></i></div><p>前后测使用同一量表；分数变化仅作为本次状态记录。</p></div><div class="report-section-title"><h3>01 · 生理信号与情绪迁移</h3><span class="legend">● 前测 <span>● 后测</span> · DEMO 模拟数据</span></div><div class="signal-grid">${signal(b,a,'heartRate','心率','次/分钟')}${signal(b,a,'respiration','呼吸','次/分钟')}${signal(b,a,'temperature','体温','°C')}<article class="panel emotion-panel"><h3>情绪迁移 <small>前测 → 后测</small></h3><p>情绪极性 <strong>${num(b.emotion.polarity)} → ${num(a.emotion.polarity)}</strong></p>${[['happy','愉悦'],['neutral','中性'],['sadness','低落'],['fear','恐惧'],['surprise','惊讶'],['disgust','厌恶']].map(([k,l])=>`<div class="emotion-row"><span>${l}</span><div>${[b,a].map(d=>`<i style="width:${Math.max(0,Math.min(1,d.emotion[k]||0))*100}%"></i>`).join('')}</div><small>${num(b.emotion[k])} → ${num(a.emotion[k])}</small></div>`).join('')}<p class="signal-note">缺失项显示 —，不推断情绪诊断。</p></article></div><div class="report-section-title"><h3>02 · EEG 分频段趋势</h3><span>各频段独立展示</span></div><div class="eeg-grid">${bands.map(([k,l])=>signal(b,a,k,l,'设备字段值')).join('')}</div><details class="report-method"><summary>数据说明与比较方式</summary><p>演示曲线来自内置模拟样本。正式接口兼容平台导出的 label / preValue / dateValues 结构；Alpha、Beta、Gamma 的分量字段分别映射展示。</p></details>`;
}
