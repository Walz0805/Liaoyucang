...
for(const label of ['心率','呼吸','体温','情绪分布','Delta','Theta','Low Alpha','High Alpha','Beta','Gamma','最大值','末次值','最小值'])assert(panels.includes(label));
...
await click({action:'select'});await click({scene:'forest'});await click({music:'pop'});await click({action:'confirm-plan'});assert(elements.get('#app').innerHTML.includes('林间微光'));assert(elements.get('#app').innerHTML.includes('流行音乐'));
