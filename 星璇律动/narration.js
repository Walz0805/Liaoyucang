import {followVoice,stopMouth} from './lipsync.js';
export const narrationFiles={consent:'consent',scan:'baseline',prepare:'prepare',select:'select',after:'after',report:'report',C01:'guide',F01:'wake'};
...
export function playNarration(key){
 const file=narrationFiles[key];if(!file||typeof Audio==='undefined')return;
 stopNarration();const id=generation;audio=new Audio(`assets/voice/${file}.wav`);audio.volume=volume;const player=audio;player.id="stage-narration";player.hidden=true;document.body.appendChild(player);
 control().hidden=false;button.textContent='正在载入讲解…';player.addEventListener('ended',()=>{if(id===generation)button.textContent='↻ 重听讲解';});attempt(player,id);
}
