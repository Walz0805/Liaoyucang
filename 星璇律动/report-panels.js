...
export function reportInsight(b,a){
 ...
 return {analysis:changes.length?changes.join('，')+'。这些是两次采集之间的数值差异，不能单独说明疗效。':'当前可比较的数据不足，暂不生成变化结论。',suggestion:a.flags.fatigue?'后测仍带有疲劳标记。下次可继续选择低刺激、慢节奏的声景，并结合自身感受与工作人员讨论是否调整时长。':'下次可以保留喜欢的场景；如果体验不合适，优先调整音乐或缩短时长，并向工作人员反馈。'};
}
export function reportPanels(b,a){
 ...
 return `<div class="report-section-title"><h3>01 · 生理信号与情绪</h3><span class="legend">● 前测 <span>● 后测</span> · DEMO 模拟数据</span></div><div class="signal-grid">${signal(b,a,'heartRate','心率','次/分钟')}${signal(b,a,'respiration','呼吸','次/分钟')}${signal(b,a,'temperature','体温','°C')}<article class="panel emotion-panel"><h3>情绪分布 <small>字段值</small></h3><p>情绪极性 <strong>${num(b.emotion.polarity)} → ${num(a.emotion.polarity)}</strong></p>${[['happy','愉悦'],['neutral','中性'],['sadness','低落'],['fear','恐惧'],['surprise','惊讶'],['disgust','厌恶']].map(([k,l])=>`<div class="emotion-row"><span>${l}</span><div>${[b,a].map(d=>`<i style="width:${Math.max(0,Math.min(1,d.emotion[k]||0))*100}%"></i>`).join('')}</div><small>${num(b.emotion[k])} / ${num(a.emotion[k])}</small></div>`).join('')}<p class="signal-note">缺失项显示 —，不推断情绪诊断。</p></article></div><div class="report-section-title"><h3>02 · EEG 分频段趋势</h3><span>各频段独立展示</span></div><div class="eeg-grid">${bands.map(([k,l])=>signal(b,a,k,l,'设备字段值')).join('')}</div><details class="report-method"><summary>数据说明与比较方式</summary><p>演示曲线来自内置模拟样本。前后测按各自采样进程对齐，非同一时刻。未提供时间戳与 EEG 单位，不补造时间或百分比；Low / High Alpha 使用独立字段。</p></details>`;
}
